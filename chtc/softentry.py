...
class SoftEntry(CaseInsensitiveDict):
    ...
    required_fields = ["software", "version"]

    def __init__(self, *args, **kwargs):
        CaseInsensitiveDict.__init__(self, *args, **kwargs)
        if len(args) == 0:
            raise TypeError("Can't initialize as empty")
        print set(SoftEntry.required_fields) - set(self.keys())
        if len(set(SoftEntry.required_fields) - set(self.keys())) > 0:
            fields = ', '.join(['"'+s+'"' for s in SoftEntry.required_fields])
            raise TypeError("One of the following fields missing: " + fields)

class DepSoftEntry(SoftEntry):
    def __init__(self, *args, **kwargs):
        SoftEntry.__init__(self, *args, **kwargs)
        if set(self.keys()) != set(SoftEntry.required_fields):
            raise TypeError("Dependent Software Entry consists of 'Software' and 'Version' fields")

def __run_tests():
    try:
        x = SoftEntry({'Software': 'python'})
